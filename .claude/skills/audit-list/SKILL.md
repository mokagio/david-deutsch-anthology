...
When a rejected entry is confirmed, add its durable title or show rule to `/find-entries`' shared `ignore.yml`; that prevents rediscovery and lets future audits detect it if it returns to `list.yml`.
